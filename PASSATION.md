...
0 sexies. ✅ **LE CHANTIER « PREMIER DÉMARRAGE » EST LIVRÉ** *(2026-09-10)*.
   Fusionné (`6718bf7`), poussé, **construit et installé à 10:46**, migration
   **024** appliquée à la vraie base — qui est en **version 24**,
   `integrity_check` ok, 0 violation de clé étrangère, **13 notes / 51 761
   octets inchangés**. ⭐ **UN SEUL BUILD pour la facturation ET ce chantier**,
   comme convenu. Ligne de base après fusion : **1012 tests**, tout vert.
   ⛔ **Il reste un geste HUMAIN : la fenêtre de trousseau.** Le binaire a
   changé, macOS redemande l'accès — Antonin doit cliquer « Toujours
   autoriser ». ⚠️ Il ne verra PAS l'accueil au lancement (son drapeau a été
   repris, c'est voulu) : **Réglages → « Refaire l'accueil du premier
   démarrage »**, puis relancer. Détail : `PASSATION-ONBOARDING.md`.
